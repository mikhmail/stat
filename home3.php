...
<tr><td><p>
Найдено <b><font color="red"><?=$count?> </font></b> аппарата.<br>
		...
		<div style="float: left; width: 20%;">
			<ul style="text-align: left;">
				<?foreach ($row_global1 as $aparat_name => $value) {
					...
					<a href="index.php?id_aparat=<?=$value[0]["id_aparat"]?>&id_sc=<?=$id_sc_now?>"><b><?=$aparat_name?></b><font color="red"><sup><?=count($value)?></sup></font></a>
				<?}?>
			</ul>
		</div>
...
<table align="center" border="1" width="80%">
	<tr>
		<th><b>№</b></th>
		<th><b>Дата приемки</b></th>
		<th><b>Дата выдачи</b></th>
		<th><b>Ремонт</b></th>
...
?>
	<tr style="padding:10px;">
		<td style="padding:10px;"><a href="<?=$_SERVER['PHP_SELF']?>?nomer_kvitancy=<?=$row['nomer_kvitancy']?>"><?=$row['nomer_kvitancy']?></a></td>
		<td style="padding:10px;"><p><span id="app_<?=$row['id_kvitancy']?>" <?=$span?>><font size="1"><?=$row['date_priemka']?></font></span></p></td>
		<td style="padding:10px;"><p><span id="app_<?=$row['id_kvitancy']?>" <?=$span?>><font size="1"><?if ($row['date_vydachi'] == '0000-00-00') {$row['date_vydachi'] = 'Не выдан';}?><?=$row['date_vydachi']?></font></span></p></td>
		<td style="padding:10px;">
		
		<a href="#" <?=$span?> onclick="anichange(this); return false">
...
?>
	...
	<?=$row['model']?></b> - <?=$row['neispravnost']?>
	<sup><b><?=diff($row['date_priemka'])?> days</b></sup>
...
<input id="sms_<?=$row['id_kvitancy']?>" type="button" value="sms">
