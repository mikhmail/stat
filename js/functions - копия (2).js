...
function ShowSklad(vendor_id, text_id, id){
	
	
	var sel = document.getElementById(vendor_id); // Получаем наш список
	var vendor_id = sel.options[sel.selectedIndex].value; // Получаем текст нашей опции
	
	var text=document.getElementById(text_id).value;
	
	$.post("showSklad.php", { vendor:vendor_id, request:text })
	.done(function(data) {
	$('div#zap_rez_'+id+'').html(data);
