...
//$arr_month[] = '0'.$i;
	...
	if($i != $id_month ) {
		$where1 .= "(SELECT COUNT(*) FROM kvitancy WHERE date_priemka between '$id_year-0$i-01' and '$id_year-0$i-31') AS cnt_$i,";
	} else {
		$where1 .= "(SELECT COUNT(*) FROM kvitancy WHERE date_priemka between '$id_year-0$i-01' and '$id_year-0$i-31') AS cnt_$i";
	}
}
...
for ($i=1; $i<=$id_month; $i++) {
	...
	if($i != $id_month ) {
		$where2 .= "(SELECT COUNT(*) FROM kvitancy WHERE id_sost in (6, 7) AND date_priemka between '$id_year-0$i-01' and '$id_year-0$i-31') AS cnt_$i,";
	} else {
		$where2 .= "(SELECT COUNT(*) FROM kvitancy WHERE id_sost in (6, 7) AND date_priemka between '$id_year-0$i-01' and '$id_year-0$i-31') AS cnt_$i";
	}
}
...
for ($i=1; $i<=$id_month; $i++) {
	...
	if($i != $id_month ) {
		$where3 .= "(SELECT COUNT(*) FROM kvitancy WHERE id_sost in (4, 8, 9) AND date_priemka between '$id_year-0$i-01' and '$id_year-0$i-31') AS cnt_$i,";
	} else {
		$where3 .= "(SELECT COUNT(*) FROM kvitancy WHERE id_sost in (4, 8, 9) AND date_priemka between '$id_year-0$i-01' and '$id_year-0$i-31') AS cnt_$i";
	}
}
...
// Dataset definition   
 ...
 $DataSet->SetSerieName("Без ремонта","Serie4");  
 ...
 // Draw the title  
 $Title = "Общее число приемок ежемесячно";  
