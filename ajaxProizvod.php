...
$login = Login::getInstance();
    ...
    if(isset($_POST['queryString'])) {
        ...
        $row = Filter::select( "Select *  From proizvoditel where name_proizvod LIKE UPPER('".$_POST['queryString']."%') order by name_proizvod asc" );
        if($row) {
            
		$div = "\r\n<div  style='position:absolute; background:#fff; overflow:auto; height:99px; width:500px; z-index:1; margin-top:7px; margin-left:0px; border:1px solid #5aa8cc; border-radius: 6px; -webkit-border-radius: 6px; -moz-border-radius: 6px; padding:6px; ' id='proizvod_box' >\r\n\r\n<div  style='position:relative; cursor:pointer; color:#def0f8; font:bold 16px Arial; height:15px; width:46px; z-index:2; margin:2px 0 -15px 398px; border:0px solid red; text-shadow:#162b35 2px 2px 3px;' onclick=\"document.getElementById('proizvod_box').style.display='none';\">закрыть</div>\r\n<ul class='".__FILE__."'>";
